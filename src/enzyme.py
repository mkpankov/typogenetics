...
class Enzyme:
    """It's a machine operating on strands by means of
    instructions--Amino Acids.
    """
    ...
    def translate(self):
        ...
        for a in self.commands:
            ...
            if c == 'pun':
                ...
                self.production, self.locus = f(self.production,
                                                self.locus,
                                                self.copy,
                                                self.active_strand)
            elif c == 'lcs':
                self.locus = f(self.production,
                               self.locus,
                               self.copy,
                               self.active_strand)
            elif c == 'md-':
                ...
                if old == False and self.copy == True:
                    self.production.append(
                        aminoacid.complement(
                            self.production[self.active_strand],
                            self.locus,
                            self.locus + 1))
            elif c == 'ast':
                ...
                if old != self.active_strand:
                    assert len(self.production[0]) == len(
                        self.production[1])
            ...
            if self.locus < 0 or\
               self.locus >= len(self.production[self.active_strand]):
                raise OffStrand
        yield self.production
