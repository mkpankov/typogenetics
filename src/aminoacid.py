...
def ensure_complement(active, strands, start=0, stop=None):
    ...
    other = swi(active)
    try:
        strands[other].extend(
            [' '] * (len(strands[active]) - len(strands[other])))
        strands[other][start:stop] = complement(
            strands[active], start, stop)[start:stop]
    except IndexError:
